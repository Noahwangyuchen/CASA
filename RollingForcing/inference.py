...
parser.add_argument("--text_encoder_cpu_offload", action="store_true",
                    help="Keep text encoder on CPU and move embeds to GPU to save VRAM")
...
if args.lora_path:
    ...
    lora_state = load_lora_state_dict(args.lora_path)
    applied, missing = merge_lora_into_model(
        pipeline.generator.model, lora_state, scale=args.lora_scale
    )
    if local_rank == 0:
...
if getattr(config, "text_encoder_cpu_offload", False):
    ...
    pipeline.text_encoder.set_output_device(device)
