...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        t5_safetensors = os.path.join(
            MODEL_FOLDER, "Wan2.1-T2V-1.3B", "models_t5_umt5-xxl-enc-bf16.safetensors"
        )
        t5_pth = os.path.join(
            MODEL_FOLDER, "Wan2.1-T2V-1.3B", "models_t5_umt5-xxl-enc-bf16.pth"
        )
        if os.path.exists(t5_safetensors):
            t5_state = safe_load_file(t5_safetensors, device="cuda")
        else:
            t5_state = torch.load(t5_pth, map_location="cuda")
        self.text_encoder.load_state_dict(t5_state)
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        with torch.device("meta") if meta_init else nullcontext():
            if skip_base_weights:
                if "14B" in model_name:
                    from wan.configs.wan_t2v_14B import t2v_14B as model_cfg
                else:
                    from wan.configs.wan_t2v_1_3B import t2v_1_3B as model_cfg
                if is_causal:
