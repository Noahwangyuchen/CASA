...
def load_transformer(config, meta_transformer=False):
    ...
    skip_base_weights = bool(config.get("skip_base_weights", False))
    transformer = WanDiffusionWrapper(
        model_name=model_name,
        timestep_shift=timestep_shift,
        is_causal=True,
        skip_base_weights=skip_base_weights,
    )
    ...
    if lora_path:
        ...
        applied, missing = merge_lora_into_model(
            transformer.model, lora_state, scale=lora_scale
        )
        log.debug(f"LoRA merge applied: {len(applied)} layers, missing: {len(missing)}")
